...
Разделение данных проводится по схеме 80% (обучение) и 20% (валидация). Бейзлайном выступает предобученная модель YOLOv12m от Ultralytics и OCR-модель TPS-ResNet-BiLSTM-Attn без дообучения. Изображения проходят стандартизированную предобработку (масштабирование).

- Использование EfficientDet и Transformer-based OCR рассматривается как направление для будущих итераций, помимо текущего этапа. Планируется также улучшать аугментации и оптимизировать гиперпараметры.
...
Планируется провести пилотную демонстрацию системы распознавания координат на выделенном сервере с веб-интерфейсом. Пользователь сможет запустить обработку тестового видео, кадры которого будут последовательно передаваться в пайплайн:

- модель **YOLO** (дообученная) будет выполнять детекцию координатных столбиков;
- модель **EasyOCR** (дообученная) — распознавание символов координат.

Ожидается, что результаты будут отображаться в веб-интерфейсе в режиме, близком к реальному времени: координаты и зоны детекции будут визуализированы поверх видео.

Предусматривается логирование и мониторинг работы системы. Для каждого кадра планируется сохранять:

- распознанную координату;
- метку времени;
- время обработки кадра.

Для мониторинга предполагается использовать **MLflow** (отслеживание метрик и логов) и **Grafana** (наблюдение за загрузкой ресурсов и производительностью в реальном времени). Интерфейс будет дополнительно отображать текущие значения метрик — точность, скорость обработки, стабильность.

---

### 3.2 Способ оценки пилота

Оценка успешности пилота будет производиться путём сравнения результатов системы с эталонной разметкой тестового видео. Планируется использовать следующие метрики:

- **Бейзлайн:** модели *yolo12m* и *TPS-ResNet-BiLSTM-Attn* без дополнительного дообучения.
- **Точность OCR (accuracy):** ≥ 90% — доля правильно распознанных символов или строк координат.
- **Точность определения координат:** ≥ 90% — доля кадров, где координаты распознаны полностью и верно.
- **Среднее время обработки кадра:** < 1 секунды — средняя задержка на обработку одного кадра от получения до результата.

Метрики будут рассчитаны посткадрово, на основе сопоставления распознанных координат с эталонными. Замеры времени обработки будут выполняться через встроенное логирование и средства мониторинга. Пилот будет считаться успешным при достижении всех указанных порогов.

---

### 3.3 Подготовительные шаги

Для проведения пилота планируется выполнить следующие подготовительные действия:

- **Развертывание моделей.** Загрузить дообученные модели YOLO и EasyOCR на сервер, настроить параметры детекции и предобработки изображений.
- **Подготовка тестового видео и разметки.** Выбрать подходящее видео с координатными столбиками, собрать или подтвердить эталонную разметку координат по кадрам.
- **Настройка логирования и мониторинга.** Подключить MLflow для отслеживания метрик и логов, настроить Grafana/Prometheus для визуального контроля ресурсоёмкости и производительности.
- **Разработка и тестирование веб-интерфейса.** Создать интерфейс для запуска демонстрации, отображения видео и визуализации результатов распознавания.
- **Оценка требуемых вычислительных ресурсов.** Модель тестировалась на локальной RTX 3080 (≈29 TFLOPs), инференс занимает ~30 мс на кадр. Для пилота достаточно карты уровня RTX 2060 (≈13 TFLOPs), что уменьшит затраты без потери скорости.
- **Оценка производительности.** Провести тестовые прогоны для оценки средней скорости обработки и потребления ресурсов; при необходимости — оптимизировать параметры и загрузку.
...
```mermaid
graph TD
    A[Пользователь] -- загружает видео --> B[Streamlit UI]
    B -- HTTP --> C[FastAPI сервис]
    C -- вызов моделей --> D[YOLO + OCR]
    C -- запись результатов --> E[(PostgreSQL)]
    C -- отдача данных --> B
```

Видео передается через UI в сервис FastAPI, который разбивает его на кадры,
обрабатывает модели YOLO и OCR и сохраняет результаты распознавания
в таблицу PostgreSQL. Итоговые данные отображаются пользователю в веб‑интерфейсе.

### 4.2. Инфраструктура и масштабируемость

- Сервис разворачивается в Docker‑контейнере.
- База данных PostgreSQL поднимается как отдельный контейнер.
- Веб‑интерфейс Streamlit обращается к FastAPI по внутренней сети.
- Модели хранятся локально в образе (`custom_inference/models`).
- При росте нагрузки сервис масштабируется горизонтально за счет
  запуска нескольких копий приложения за балансировщиком.

### 4.3. Требования к работе системы

- Время обработки одного кадра при работе на CPU — не более 100 мс.
- Точность OCR и детекции координат не ниже 90 %.
- Система должна обрабатывать минимум 30 кадров в секунду
  при параллельной работе нескольких пользователей.

Для обеспечения надежности используется асинхронная обработка запросов
и контроль очередей. При необходимости сервис можно масштабировать
по количеству реплик и выделять отдельный узел под базу данных.
...
- Пилотные затраты: ~100$ на аренду или покупку GPU среднего класса (например, RTX 2060) для демонстрации и отладки.

### 4.6. Нагрузочное тестирование

Для оценки пропускной способности использовался инструмент `locust`.
При 10 одновременных клиентах и загрузке видеоролика размером 5 МБ
среднее время ответа составило 1.2 с, потребление памяти не превышало 500 МБ.
Полученные результаты подтвердили возможность обработки
до 50 видео-запросов в минуту без деградации качества сервиса.
