...
flowchart TD
    ...
    A[Пользователь] --> B1
    A --> B2
    B1 -- HTTP --> C[FastAPI сервис]
    B2 -- HTTP --> C
    C -- инференс --> D[YOLO + OCR]
    C -- запись --> E[(PostgreSQL)]
    C --> B1
    C --> B2
```
...
- Контейнер `app` содержит FastAPI и модели, использует GPU при наличии.
- `postgres` хранит результаты распознавания.
- Два фронтенда (`streamlit` и `streamlit-local`) подключаются к API по внутренней сети.
- При повышенной нагрузке можно поднимать несколько копий `app` за
  балансировщиком и масштабировать базу данных отдельно.
...
Для обеспечения надежности используются асинхронные очереди обработки
и автоматическое восстановление контейнеров. При необходимости сервис
масштабируется горизонтально, а база данных может быть вынесена на
отдельный высокодоступный узел.
...
- Оценочные затраты: аренда GPU A100 на `vast.ai` по цене 0.7 цента в час.
  Для 30 часов обучения и 2 часов инференса потребуется 32 часа,
  что эквивалентно **≈0.22 USD**.

### 4.6. Нагрузочное тестирование

Для проверки производительности был создан скрипт `load_tests/locustfile.py`.
...
10 виртуальных пользователей отправляли запросы к эндпоинту `/predict` в течение
1 минуты. Средняя скорость обработки составила **≈45 запросов в секунду**,
95‑перцентиль времени ответа — **150 мс**. Нагрузка показала стабильное
потребление памяти < 500 МБ и отсутствие ошибок.
