...
```mermaid
flowchart TD
    A[Пользователь] -->|загрузка видео| B(API FastAPI)
    B --> C[Инференс YOLO]
    C --> D[PostgreSQL]
    B -->|HTML| E[Веб‑интерфейс]
    D --> E
```

Видео загружается через веб-интерфейс. FastAPI обрабатывает файл и покадрово
передает кадры модулю инференса (YOLO). Результаты детекции сохраняются в
базу данных и отправляются обратно пользователю в виде аннотированных кадров.

### 4.2. Инфраструктура и масштабируемость

- Сервис разворачивается в Docker-контейнерах на сервере с GPU (например,
  NVIDIA A100 или Jetson).
- Компоненты: контейнер с FastAPI и моделью, отдельный контейнер с PostgreSQL,
  опционально Nginx как обратный прокси.
- Масштабирование достигается горизонтальным развертыванием нескольких копий
  контейнера FastAPI с балансировкой нагрузки.

### 4.3. Требования к быстродействию и надежности

- Обработка одного кадра не более 100 мс (≈10 FPS).
- Сервис должен автоматически перезапускаться при сбоях (Docker restart
  policy) и иметь `/health` эндпоинт для мониторинга.
- При росте нагрузки сервис масштабируется за счет увеличения числа
  экземпляров и вынесения инференса в отдельные воркеры.

### 4.4. Безопасность системы

- Вся обработка выполняется локально; данные не отправляются во внешние сети.
- Система выступает советчиком, окончательное решение принимает человек.

### 4.5. Безопасность данных

- Данные хранятся в локальной PostgreSQL, доступ ограничен внутренней сетью.

### 4.6. Издержки

- Пилотные затраты: около 100$ на аренду GPU для разработки и демонстрации.

### 4.7. Integration points

- REST эндпоинты FastAPI (`/api/inference/*`, `/api/trip/*`).
- Веб-интерфейс для загрузки видео и просмотра результатов.

### 4.8. Риски

- Снижение точности при плохой погоде или отсутствии видимости столбиков.
- Возможные проблемы с производительностью на слабом железе.

***Риски могут привести к некорректному определению координат и ложным предупреждениям.***

### 4.9. Нагрузочное тестирование

Тестирование проводилось инструментом `ab` (Apache Bench) на локальном сервере.
10 одновременных клиентов отправляли по 20 запросов к эндпоинту `/api/inference/video`.
Среднее время ответа составило около 150 мс, ошибки не зафиксированы.
